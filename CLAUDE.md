...
- **Tests:** `test-smoke.html` is the whole harness (105 assertions, no test runner, no framework). It renders `ALL-PASS` or `FAILURES: <n>` into `<pre id="out">`. Serve, then run headless Chrome. This is a **Windows** box; both Chrome and Edge are installed. Run it from the Bash tool (the pipeline below is POSIX `sed`, not PowerShell):
  ```bash
...
Sprints 1–7 are complete. Next is **Sprint 8** (PHP API + MySQL: missions, progress with server-side validation, leaderboard). Sprint order lives in [README.md](README.md) and [docs/DESIGN.md](docs/DESIGN.md).
...
**Sprint 6 — path orders.** [js/systems/CommandSystem.js](js/systems/CommandSystem.js) executes the per-operator order queue (`op.orders`, data on the operator) at the brief's slot 4: after Movement, before DoorSystem. Node types: `MOVE`, `DOOR` (delegates to `DoorSystem.orderDoorAction` — DoorSystem remains the sole door executor), `WATCH` (turn toward a point, `OPERATOR_TURN_RATE`; CombatSystem overrides `direction` while engaged, so the node finishes after contact ends — intended), `STOP` (hold until the GO signal; `go()` releases every waiting operator in the same frame — a go-code for synchronized entries). Sprint 7 added the `FLASH` node and the `BREACH` door action. **A MOVE node's path is computed when the node activates, not when queued** — so a MOVE queued behind a DOOR node paths through the just-opened door; the `preview` polyline computed at queue time is for plan rendering only and may be null (e.g. target behind a closed door — FogRenderer falls back to a straight dashed line). Input: right-click replaces the queue, Shift+right-click appends, Alt+right-click appends a WATCH node, `S` appends a STOP, `G` fires GO (EXECUTING only — a GO pressed in PLANNING must not linger). Order-node markers (MOVE circle, DOOR square, WATCH arrow, STOP diamond) are drawn by `FogRenderer._drawPlan` above the fog. A corpse drops its queue (`CombatSystem._applyDamage` and a guard in `CommandSystem.update`).

**Sprint 7 — gadgets.** One gadget slot per operator (`op.gadget = {type, uses}`, brief #9), assigned by `Game` from the map's optional `player_spawn[].gadget` (validated; default alternates FLASHBANG / BREACH_CHARGE; supply in `CFG.GADGET_COUNT`). [js/systems/GadgetSystem.js](js/systems/GadgetSystem.js) owns thrown flashbangs: flight over `FLASH_FUSE_S`, then detonation — stun `FLASH_STUN_S` within `FLASH_RADIUS_TILES` **with an LOS test from the blast point** (a wall blocks the flash, a window doesn't) applied to **both sides** (a careless throw stuns your own team), plus `raiseNoise(FLASH_ALERT_TILES)` through walls. Throws are rare events, so grenades/flashes may allocate — unlike tracers. The `FLASH` order node (`queueFlash`, key `F` at the cursor) throws when within `FLASH_THROW_TILES` + LOS, otherwise walks the operator closer and throws en route. `BREACH` is a third `DoorSystem` action (Alt+right-click on a closed door): requires a `BREACH_CHARGE` with uses left (`orderDoorAction` rejects otherwise), plants for `BREACH_PLANT_S`, then sets the door to `breached` (permanent), consumes the charge, `raiseNoise(BREACH_ALERT_TILES)`, and stuns in a `BREACH_CONE_DEG` cone from the door center pointing away from the operator (`_breachStun` runs **after** `setState`, so the LOS test works through the fresh breach). **STUNNED contract** (`entity.stunTimer`, ticked in `Entity.update`): no movement (path frozen), no shooting (`CombatSystem` clears `combatTarget`), blind (`DetectionSystem` skips stunned viewers — deliberately, flashbangs blind), no decisions (`AISystem`), plan/door timers frozen (`CommandSystem`/`DoorSystem`); noise still alerts the stunned (state changes, actions don't). Operators keep their fog-of-war vision while stunned (UX decision — the squad's map knowledge doesn't blink). `EntityRenderer` draws flying grenades, the expanding flash, and a dashed yellow stun ring.
