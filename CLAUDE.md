...
Sprints 1–3 are complete: map + camera + operator with pathfinding; enemies (`js/entities/Enemy.js`), `js/systems/DetectionSystem.js` (vision cone + LOS raycast over `losMask` via `MapData.castRay`/`hasLineOfSight`), `js/systems/AISystem.js` (PATROL/IDLE/ALERT with alert propagation); and fog of war — `js/systems/VisionSystem.js` (player-team visibility polygons: FOV cone + `PROXIMITY_VISION_TILES` close ring, plus the visible-enemies set; computed in the **render** phase so fog works in PLANNING too) and `js/rendering/FogRenderer.js` (three-state fog on `#fog`: unexplored opaque / explored dimmed via a persistent offscreen `explored` canvas / in-view clear; order paths are drawn **above** the fog so plans into unexplored rooms stay visible). Enemies outside player vision are skipped in `EntityRenderer` (fog dims terrain only — it does not hide entities by itself in explored areas). Next is Sprint 4 (doors + mask sync — `MapData.rebuildMasks` and `MapRenderer.markDirty` already exist for this). Sprint order is in README.md.
